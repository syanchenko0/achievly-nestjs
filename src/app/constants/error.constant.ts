export const ZOD_ERROR = {
  required_error: 'Обязательное поле',
  invalid_type_error: 'Введите корректное значение',
};

const WRONG_BODY = 'Некорректное тело запроса';

const WRONG_PARAMS = 'Некорректные параметры запроса';

const WRONG_TOKEN = 'Неправильный токен';

const USER_NOT_FOUND = 'Пользователь не найден';

const GOAL_NOT_FOUND = 'Цель не найдена';

const GOAL_UPDATE_FORBIDDEN = 'Редактирование цели недоступно для пользователя';

const GOAL_DELETE_FORBIDDEN = 'Удаление цели недоступно для пользователя';

const TASK_NOT_FOUND = 'Задача не найдена';

const TASK_UPDATE_FORBIDDEN =
  'Редактирование задачи недоступно для пользователя';

const TASK_DELETE_FORBIDDEN = 'Удаление задачи недоступно для пользователя';

const TEAM_NOT_FOUND = 'Команда не найдена';

const TEAM_NOT_JOINED = 'Пользователь не состоит в команде';

const TEAM_ALREADY_JOINED = 'Пользователь уже состоит в команде';

const TEAM_DELETE_FORBIDDEN = 'Удаление команды недоступно для пользователя';

const TEAM_FORBIDDEN = 'Команда недоступна для пользователя';

const MEMBER_NOT_FOUND = 'Участник команды не найден';

const MEMBER_UPDATE_FORBIDDEN =
  'Редактирование участника недоступно для пользователя';

const MEMBER_DELETE_FORBIDDEN =
  'Удаление участника недоступно для пользователя';

const PROJECT_CREATE_FORBIDDEN = 'Создание проекта недоступно для пользователя';

const PROJECT_NOT_FOUND = 'Проект не найден';

const PROJECT_TASK_NOT_FOUND = 'Задача проекта не найдена';

const PROJECT_FORBIDDEN = 'Проект недоступен для пользователя';

const PROJECT_UPDATE_FORBIDDEN =
  'Редактирование проекта недоступно для пользователя';

const BUDGET_NOT_FOUND = 'Бюджет пользователя не найден';

const COLUMN_IN_USE = 'В столбце есть задачи';

export {
  WRONG_BODY,
  WRONG_PARAMS,
  WRONG_TOKEN,
  USER_NOT_FOUND,
  GOAL_NOT_FOUND,
  GOAL_UPDATE_FORBIDDEN,
  GOAL_DELETE_FORBIDDEN,
  TASK_NOT_FOUND,
  TASK_UPDATE_FORBIDDEN,
  TASK_DELETE_FORBIDDEN,
  TEAM_NOT_FOUND,
  TEAM_NOT_JOINED,
  TEAM_ALREADY_JOINED,
  TEAM_DELETE_FORBIDDEN,
  TEAM_FORBIDDEN,
  MEMBER_NOT_FOUND,
  MEMBER_UPDATE_FORBIDDEN,
  MEMBER_DELETE_FORBIDDEN,
  PROJECT_CREATE_FORBIDDEN,
  PROJECT_NOT_FOUND,
  PROJECT_TASK_NOT_FOUND,
  PROJECT_FORBIDDEN,
  PROJECT_UPDATE_FORBIDDEN,
  BUDGET_NOT_FOUND,
  COLUMN_IN_USE,
};
